...
(function _wireEnrichedListener() {
    ...
    const ET_OFFSET_S = 14400;  // 4hr EDT, matches AtraxLiveChart's bar-time convention
    window.AltarisEvents.on('data:candle:enriched', (ev) => {
        try {
            ...
            const etSec = utcSec - ET_OFFSET_S;
            window._bpByBar[etSec] = ev.bp;
            // Trim to last 60min — keep bp lookup bounded
            const keys = Object.keys(window._bpByBar);
            if (keys.length > 200) {
                const cutoff = etSec - 3600;
                for (const k of keys) {
                    if (parseInt(k, 10) < cutoff) delete window._bpByBar[k];
                }
            }
        } catch (_) {}
    });
...
class BigPrintBubbleRenderer {
    ...
    draw(target, priceConverter) {
        ...
        try { target.useMediaCoordinateSpace(({ context: ctx, mediaSize }) => {
            ...
            // ⚠ Timezone: bar.originalData.time is in ET seconds (LWC convention,
            // post ET_OFFSET_SEC=14400 subtraction in AtraxLiveChart.onHistory).
            ...
            const ET_OFFSET_MS = 14400 * 1000;  // 4hr EDT — matches index.html
            const sortedBars = [];
            for (let i = from; i < to; i++) {
                const bar = d.bars[i];
                if (!bar) continue;
                // ⚠ LWC CUSTOM SERIES QUIRK: bar.time is the BAR INDEX (97, 98, 99…),
                // NOT a timestamp. The actual UNIX time we fed via setData lives in
                // bar.originalData.time. Reading bar.time first treats indices as
                // seconds → binary search compares 97 vs 1.7B, drops everything as
                // "past tolerance." Read originalData.time first, fall back to
                // bar.time only if needed (rare).
                const tSec = bar.originalData?.time || bar.time || 0;
            ...
            for (const k in barSignalMap) {
                ...
                if (!sig || sig.symbol !== sym) continue;
                const bar = _findContainingBar(((sig.t || 0) * 1000));
                if (!bar) continue;
                const barX = bar.x;
                const ts = sig.t || 0;
                // ────────────────────────────────────────────────────────────
                // ABSORPTION — context layer, NOT a directional signal.
                //
                // Backtest verdict (2026-04-27, n=522 fires, 1m horizon):
                //   only 5.4% of fires actually held/consolidated at the level
                //   33.9% next bar abandoned the level entirely
                //   35.2% broke through DOWN, 25.5% moved UP
                //   1m WR=52.4% — barely above coin flip
                //
                // Conclusion: absorption identifies WHERE MM was active, not
                // which way price will go next. We removed the bullish/bearish
                // arrow (↑/↓) because that arrow implied a direction the data
                // doesn't support. Label now reads "DEF {vol}" = "this level
                // saw {vol} traded with refill." Colour stays single-tone gold.
                // ────────────────────────────────────────────────────────────
                if (sig.absorption && sig.absorption.length > 0 && settings.showRealAbs) {
                    for (const abs of sig.absorption.slice(0, 1)) {
                        const refillTag = (abs.refill_class === 'instant' || abs.refill_class === 'fast') ? '·R' : '';
                        _flatSigs.push({
                            type:    'absorption',
                            ts:      ts,
                            x:       barX,
                            price:   abs.price,
                            label:   `DEF ${_fmtVol(abs.volume)}${refillTag}`,
                            color:   COLORS.real_abs,
                            volume:  abs.volume,
                            strength: abs.strength || 0,
                            extreme: (abs.strength || 0) >= 1.8,
                            borderW: 2.5,
                        });
                    }
                }
            ...
            // Sort: most recent first, ties broken by strength desc
            _flatSigs.sort((a, b) => (b.ts - a.ts) || (b.strength - a.strength));

            // ════════════════════════════════════════════════════════════════
            // PER-BAR DEDUP (replaces aggressive spatial-radius dedup)
            //
            // Old logic killed every bubble whose center was within 1.4× radius
            // of any already-rendered same-type bubble. At narrow zoom (3+ hr
            // visible), 140+ ATM-clustered absorption bubbles all collapsed
            // into ONE because they shared similar Y. The user saw a single
            // gold dot for an entire morning of activity.
            //
            // New logic: at most ONE bubble per (bar_x, type) pair, but DO
            // allow many bubbles across different bars to overlap visually.
            // Many overlapping gold discs = many MM-defended levels — that's
            // the desired read. We use a 4-px X bucket so adjacent narrow-zoom
            // bars don't double-render. Cap raised to 200 so a full ATM
            // session paints across the whole visible window.
            // ════════════════════════════════════════════════════════════════
            const _renderedSigs = [];
            const _seenBuckets = new Set();   // `${type}|${xBucket}` keys
            const MAX_BAR_BUBBLES = 200;
            const X_BUCKET_PX = 4;
