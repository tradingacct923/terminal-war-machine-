...
const FEATURES = {
  ...
  flow:     { label: 'FLOW',      icon: '\u{1f4b0}', desc: 'Signed Δ Notional Flow (0DT-Hero-style) — SPX/SPY/QQQ' },
  ...
  walls8:   { label: 'WALLS 8',   icon: '\u{1f6a7}', desc: 'Per-Ticker Gamma Walls — 8 NQ single-name components' },
  dprints:  { label: 'DPRINTS',   icon: '\u{1f50e}', desc: 'Dealer Prints — raw capture (no thresholds)' },
  ccs:      { label: 'CONVICTION', icon: '\u{1f3af}', desc: 'Composite Conviction Score — directional bias framework (regime + flow + MM)' },
  svi:      { label: 'SVI',       icon: '\u{1f4c9}', desc: 'SVI Vol Surface Residual — Gatheral parametric fit + per-strike mispricing (Phase 20A)' },
  // ── 2026-05-01: registered 11 previously orphaned panes (built but not exposed via dropdown) ──
  sweep:    { label: 'SWEEP',     icon: '\u{26a1}',  desc: 'Multi-Strike Sweep Detector — 3+ adjacent option prints walking strikes within 500ms' },
  pin:      { label: 'PIN',       icon: '\u{1f3af}', desc: 'Pin Convergence — EOD pin probability per strike + expected close target' },
  hedgefc:  { label: 'HEDGE FC',  icon: '\u{1f52e}', desc: 'Hedge Forecaster — projected dealer hedge flow over 5/15/30 min from Γ × spot velocity' },
  vixterm:  { label: 'VIX TERM',  icon: '\u{1f321}', desc: 'VIX Term Structure — front/spot ratios + regime classification' },
  wing:     { label: 'WINGS',     icon: '\u{1f4c8}', desc: 'Wing Tracker — large-DTE/wing flow tracking with smart-money attribution' },
  skyline:  { label: 'GAMMA SKY', icon: '\u{1f3d9}', desc: 'Gamma Skyline — per-strike Γ exposure visualization (3D Γ cityscape)' },
  warehouse:{ label: 'WAREHOUSE', icon: '\u{1f3e6}', desc: 'Dealer Warehouse — posted vs caught (size buy/sell at each book level) per contract' },
  events:   { label: 'EVENTS',    icon: '\u{1f4c5}', desc: 'Event Calendar — upcoming earnings, FOMC, CPI, NFP with countdown' },
  spxqqq:   { label: 'SPX/QQQ',   icon: '\u{1f4ca}', desc: 'SPX-vs-QQQ regime divergence comparator (cross-asset confirm/divergence)' },
  mmatt:    { label: 'MM ATTRIB', icon: '\u{1f3aa}', desc: 'MM Attribution — per-venue NBBO ownership + capture/post tracking per contract' },
  walls8:   { label: 'WALLS 8',   icon: '\u{1f6a7}', desc: 'Per-Ticker Gamma Walls (Mag-8 + QQQ + SPX + VIX)' },
};
